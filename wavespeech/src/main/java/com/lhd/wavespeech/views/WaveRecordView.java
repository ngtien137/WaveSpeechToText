...
public class WaveRecordView extends View {
    ...
    public WaveRecordView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public WaveRecordView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        final TypedArray a = getContext().obtainStyledAttributes(
                attrs, R.styleable.WaveRecordView, defStyle, 0);

        mMode = a.getInt(R.styleable.WaveRecordView_mode, MODE_PLAYBACK);

        float strokeThickness = a.getFloat(R.styleable.WaveRecordView_waveformStrokeThickness, 5f);
        int mStrokeColor = a.getColor(R.styleable.WaveRecordView_waveformColor,
                ContextCompat.getColor(context, R.color.colorWhite));
        int mFillColor = a.getColor(R.styleable.WaveRecordView_waveformFillColor,
                ContextCompat.getColor(context, R.color.colorWhite));
        int mMarkerColor = a.getColor(R.styleable.WaveRecordView_playbackIndicatorColor,
                ContextCompat.getColor(context, R.color.colorWhite));
        int mTextColor = a.getColor(R.styleable.WaveRecordView_timecodeColor,
                ContextCompat.getColor(context, R.color.colorWhite));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mMode == MODE_RECORDING && temp != null) {
            ...
            for (float[] p : temp) {
                ...
                while (j < p.length) {
                    if (p[j + 1] != 0f) {
                        canvas.drawLine(p[j], p[j + 1], p[j], 2 * centerY - p[j + 1], mStrokePaint);
                    }
                    j += 2;
                }
